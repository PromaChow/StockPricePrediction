...
def test_fill_missing_values_basic():
    ...
    sample_data = pd.DataFrame({"A": [np.nan, np.nan, 3, np.nan, 5], "B": [np.nan, 2, np.nan, 4, 5]})
...
def test_fill_missing_values_all_nan_column():
    nan_data = pd.DataFrame({"A": [1, 2, 3], "B": [2, np.nan, np.nan], "C": [np.nan, np.nan, np.nan]})
...
def test_fill_missing_values_no_missing_data():
    complete_data = pd.DataFrame({"A": [1, 2, 3], "B": [4, 5, 6]})
